        ...
        <div class="container mx-auto flex flex-wrap justify-between items-center py-3 px-4 sm:px-6">
            ...
            <nav :class="{'block': mobileMenuOpen, 'hidden': !mobileMenuOpen}" class="w-full md:flex md:w-auto md:items-center mt-4 md:mt-0">
                <ul class="flex flex-col md:flex-row md:space-x-6 items-start md:items-center text-lg space-y-2 md:space-y-0">
                    ...
                    @if(Route::has('products.index')) {{-- Conditionally show if products listing page exists --}}
                    ...
                    @endif
                     {{-- <li><a href="{{ route('partner.landing') }}" @click="mobileMenuOpen = false" class="py-2 block hover:text-green-200 transition-colors">For Pharmacies</a></li> --}}
                </ul>
                    ...
                    @auth
                        <div x-data="{ open: false }" class="relative">
                            <button @click="open = !open" @click.outside="open = false"
                                ...
                                @if (auth()->user()->avatar_url) {{-- Assuming you have an avatar_url accessor or similar --}}
                                    <img src="{{ auth()->user()->avatar_url }}" alt="{{ auth()->user()->name }}" class="w-9 h-9 rounded-full border-2 border-green-300 object-cover">
                                @else
                                    <img src="{{ asset('images/default.jpg) }}" alt="User Avatar" class="w-9 h-9 rounded-full border-2 border-green-300 object-cover">
                                    {{-- Ensure you have a default-avatar.png in public/images --}}
                                @endif
                            ...
                            <div x-show="open" x-cloak @click.outside="open = false"
                                ...
                                {{-- Add other user-specific links here e.g. Profile, Settings --}}
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-8 mb-8">
                ...
                <div>
                    ...
                    <ul class="space-y-2 text-sm">
                        ...
                        {{-- <li><a href="{{ route('faqs') ?? '#' }}" class="hover:text-white hover:underline">FAQs</a></li> Assuming a FAQ page --}}
                    </ul>
                </div>
                <div>
                    ...
                    <ul class="space-y-2 text-sm">
                        ...
                        {{-- Add more partner-specific links --}}
                    </ul>
                </div>
                <div>
