<!-- filepath: /home/d/coder/pharmacy/resources/views/inventories/edit.blade.php -->
<x-dashboardLayout :title="'Edit Inventory'" :breadcrumb="['Inventories' => route('inventories.index'), 'Edit' => null]">
    <div class="container mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <h1 class="text-2xl font-bold text-gray-900 mb-6">Edit Inventory</h1>

        <form action="{{ route('inventories.update', $inventory->id) }}" method="POST" class="bg-white p-6 rounded-lg shadow-md space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {{-- <div>
                    ...
                    <select id="pharmacy_id" name="pharmacy_id"
                        ...
                        @foreach ($pharmacies as $pharmacy)
                            <option value="{{ $pharmacy->id }}" {{ old('pharmacy_id', $inventory->pharmacy_id) == $pharmacy->id ? 'selected' : '' }}>
                                {{ $pharmacy->name }} 
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" id="batch_number" name="batch_number" value="{{ old('batch_number', $inventory->batch_number) }}"
                ...
                <div>
                    ...
                    <input type="text" id="manufacturer" name="manufacturer" value="{{ old('manufacturer', $inventory->manufacturer) }}"
                ...
                <div>
                    ...
                    <input type="date" id="expiry_date" name="expiry_date" value="{{ old('expiry_date', $inventory->expiry_date) }}"
                ...
                <div>
                    ...
                    <input type="number" id="quantity" name="quantity" value="{{ old('quantity', $inventory->quantity) }}"
                           class="w-full border rounded-md px-3 py-2 focus:ring-2 focus:ring-green-500 focus:border-green-500 @error('quantity') border-red-500 @enderror">
                    @error('quantity')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" step="0.01" id="unit_price" name="unit_price" value="{{ old('unit_price', $inventory->unit_price) }}"
                ...
                <div>
                    ...
                    <input type="number" step="0.01" id="tax" name="tax" value="{{ old('tax', $inventory->tax) }}"
                ...
                <div>
                    ...
                    <input type="text" id="storage_location" name="storage_location" value="{{ old('storage_location', $inventory->storage_location) }}"
                ...
                <div>
                    ...
                    <select id="is_active" name="is_active" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-2 focus:ring-green-500 focus:border-green-500 @error('is_active') border-red-500 @enderror">
                        <option value="1" {{ old('is_active', $inventory->is_active) == 1 ? 'selected' : '' }}>Yes</option>
                        <option value="0" {{ old('is_active', $inventory->is_active) == 0 ? 'selected' : '' }}>No</option>
                    </select>
                    @error('is_active')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
