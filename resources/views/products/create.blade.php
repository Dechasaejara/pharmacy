<x-dashboardLayout :breadcrumb="['Products' => route('products.index'),'Create' => null]">
        ...
        <form action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded shadow-md">
            ...
            <div class="mb-4">
                ...
                <input type="text" id="generic_name" name="generic_name" 
                       class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring focus:ring-green-500 @error('generic_name') border-red-500 @enderror" 
            ...
            <div class="mb-4">
                ...
                <textarea id="description" name="description" rows="4" 
                          class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring focus:ring-green-500 @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
