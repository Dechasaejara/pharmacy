<x-dashboardLayout :title="'Edit Profile'" :breadcrumb="['Profiles' => route('profiles.index'), 'Edit' => null]">
    <div class="container mx-auto py-10 px-4 sm:px-6 lg:px-8">
        ...
        <form action="{{ route('profiles.update', $profile) }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 max-h-[70vh] overflow-y-auto pr-4">
                <!-- Full Name -->
                <div>
                    <label for="fullname" class="block text-sm font-medium text-gray-700 mb-2">Full Name <span
                ...
                @if (auth()->user()->profile->role === 'Admin')
                        ...
                        <select id="role" name="role"
                            ...
                            @foreach (['User', 'Admin', 'Pharmacist', 'Manager'] as $roleOption)
                                <option value="{{ $roleOption }}"
                                    {{ old('role', $profile->role) === $roleOption ? 'selected' : '' }}>
                                    {{ $roleOption }}</option>
                            @endforeach
                        </select>
                        @error('role')
                    ...
                    <div>
                        <label for="pharmacy" class="block text-sm font-medium text-gray-700 mb-2">Pharmacy <span
                                class="text-red-500">*</span></label>
                        <select id="pharmacy" name="pharmacy"
                            class="w-full border border-gray-200 rounded-lg px-4 py-2 text-gray-900 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors @error('pharmacy') border-red-500 @enderror"
                            >
                            <option value="" disabled>Select pharmacy</option>
                            @foreach ($pharmacies as $pharmacyOption)
                                <option value="{{ $pharmacyOption }}"
                                    {{ old('pharmacy', $profile->pharmacy) === $pharmacyOption ? 'selected' : '' }}>
                                    {{ $pharmacyOption->name }}</option>
                            @endforeach
                        </select>
                        @error('pharmacy')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <select id="gender" name="gender"
                        ...
                        <option value="" {{ old('gender', $profile->gender) === null ? 'selected' : '' }}>Select
                            Gender</option>
                        @foreach (['Male', 'Female', 'Other'] as $genderOption)
                            <option value="{{ $genderOption }}"
                                {{ old('gender', $profile->gender) === $genderOption ? 'selected' : '' }}>
                                {{ $genderOption }}</option>
                        @endforeach
                    </select>
                    @error('gender')
                ...
                <!-- Social Links -->
                <div class="md:col-span-2">
                    <label for="social_links" class="block text-sm font-medium text-gray-700 mb-2">Social Links
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (!latitudeField.value && !longitudeField.value && !countryField.value && !stateField.value && !
                cityField.value && !addressField.value) {
                fillGeolocation();
            }
            ...
            pictureInput.addEventListener('change', function(e) {
                ...
                if (file) {
                    ...
                    reader.onload = function(e) {
                        ...
                        img.classList.add('w-20', 'h-20', 'rounded-full', 'object-cover', 'mt-3',
